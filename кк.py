...
def f(x, y, a, b, c, d, e, f):
    # Эта функция будет считать новую точку нашего папоротника,по старым координатам x, y и аргуиентам a, b, c, d, e
...
RUN = True
x1, x2 = int(input('Введите число ')), int(input())
while RUN:
    ...
    P = random.randint(0, 1000)  # Изходя из вероятности будем вычислять новые координаты
    if P == 0:
